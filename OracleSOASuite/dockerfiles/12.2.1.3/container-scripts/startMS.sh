...
#Wait for startup script to be generated by admin server task
if [ ! -f $DOMAIN_HOME/bin/startManagedWebLogic.sh ]
then
  while [ ! -f $DOMAIN_HOME/bin/startManagedWebLogic.sh ]
  do  
    echo "INFO: Waiting for start up scripts to be created"
    sleep 10
  done
